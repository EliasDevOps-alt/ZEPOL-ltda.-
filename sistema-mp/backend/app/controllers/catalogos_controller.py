...
from ..models import EstadoSid, Maquina, Material, MaterialProceso, Proceso
...
def listar_materiales_por_proceso(db: Session, proceso_id: int) -> List[Material]:
    return db.scalars(
        select(Material)
        .join(MaterialProceso, MaterialProceso.material_id == Material.id)
        .where(MaterialProceso.proceso_id == proceso_id, Material.activo.is_(True))
        .order_by(Material.codigo_mp)
    ).all()
